Tonyu.klass.define({
  ...
  methods: {
    main :function _trc_Char_main() {
      ...
      //$LASTPOS=1000032;//user.Char:32
      _this.s=_this.parallel(Tonyu.bindFunc(_this,_this.scale),0.1);
      //$LASTPOS=1000055;//user.Char:55
      _this.on("fuga",function () {
        
        //$LASTPOS=1000068;//user.Char:68
        _this.print("fugaed");
      });
      //$LASTPOS=1000087;//user.Char:87
      _this.on("die",function () {
        
        //$LASTPOS=1000099;//user.Char:99
        _this.print("dead");
      });
      //$LASTPOS=1000116;//user.Char:116
      while (true) {
        //$LASTPOS=1000134;//user.Char:134
        if (_this.getkey(32)==1) {
          //$LASTPOS=1000152;//user.Char:152
          _this.die();
        }
        //$LASTPOS=1000164;//user.Char:164
        _this.update();
    ...
    go :function _trc_Char_go(speed) {
      ...
      //$LASTPOS=1000196;//user.Char:196
      while (true) {
        //$LASTPOS=1000218;//user.Char:218
        _this.print("Go Left");
        //$LASTPOS=1000244;//user.Char:244
        while (_this.x<300) {
          //$LASTPOS=1000271;//user.Char:271
          _this.x+=speed;
          //$LASTPOS=1000293;//user.Char:293
          _this.update();
          
        }
        //$LASTPOS=1000321;//user.Char:321
        _this.print("Go Right");
        //$LASTPOS=1000348;//user.Char:348
        while (_this.x>100) {
          //$LASTPOS=1000375;//user.Char:375
          _this.x-=speed;
          //$LASTPOS=1000397;//user.Char:397
          _this.update();
    ...
    scale :function _trc_Char_scale(speed) {
      ...
      //$LASTPOS=1000446;//user.Char:446
      while (true) {
        //$LASTPOS=1000468;//user.Char:468
        _this.print("Bigger");
        //$LASTPOS=1000493;//user.Char:493
        //$LASTPOS=1000498;//user.Char:498
        _this.i=0;
        while(_this.i<17) {
          {
            //$LASTPOS=1000526;//user.Char:526
            _this.scaleX+=speed;
            //$LASTPOS=1000553;//user.Char:553
            _this.update();
          }
          _this.i++;
        }
        //$LASTPOS=1000581;//user.Char:581
        _this.print("Smaller");
        //$LASTPOS=1000607;//user.Char:607
        //$LASTPOS=1000612;//user.Char:612
        _this.i=0;
        while(_this.i<17) {
          {
            //$LASTPOS=1000640;//user.Char:640
            _this.scaleX-=speed;
            //$LASTPOS=1000667;//user.Char:667
            _this.update();
          }
          _this.i++;
        }
