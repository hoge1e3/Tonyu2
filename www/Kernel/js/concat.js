...
Tonyu.klass.define({
  ...
  methods: {
    ...
    nameToRGB :function _trc_Color_nameToRGB() {
      ...
      //$LASTPOS=18004255;//kernel.Color:4255
      d=d.data;
      //$LASTPOS=18004267;//kernel.Color:4267
      if (_this.imageData) {
        //$LASTPOS=18004287;//kernel.Color:4287
        //$LASTPOS=18004291;//kernel.Color:4291
        i = 0;
        for (; i<4 ; i++) {
        ...
        //$LASTPOS=18004352;//kernel.Color:4352
        _this.value.r=d[0];
        //$LASTPOS=18004369;//kernel.Color:4369
        _this.value.g=d[1];
        //$LASTPOS=18004386;//kernel.Color:4386
        _this.value.b=d[2];
        //$LASTPOS=18004403;//kernel.Color:4403
        _this.value.a=d[3];
    ...
    makeHSLA :function _trc_Color_makeHSLA() {
      ...
      //$LASTPOS=18004441;//kernel.Color:4441
      if (_this.hasHSLA()) {
        return _this;
      }
      //$LASTPOS=18004466;//kernel.Color:4466
      if (! _this.hasRGBA()) {
        //$LASTPOS=18004482;//kernel.Color:4482
        _this.nameToRGB();
      }
      //$LASTPOS=18004497;//kernel.Color:4497
      hsl = _this.RGBToHSL(_this.value.r/_this.maxs.r,_this.value.g/_this.maxs.g,_this.value.b/_this.maxs.b);
      
      //$LASTPOS=18004563;//kernel.Color:4563
      _this.value.h=hsl[0];
      //$LASTPOS=18004581;//kernel.Color:4581
      _this.value.s=hsl[1];
      //$LASTPOS=18004599;//kernel.Color:4599
      _this.value.l=hsl[2];
      return _this;
    },
    ...
    makeName :function _trc_Color_makeName(type) {
      ...
      //$LASTPOS=18004649;//kernel.Color:4649
      if (_this.hasName()) {
        return _this;
      }
      //$LASTPOS=18004674;//kernel.Color:4674
      type=type||"rgb";
      //$LASTPOS=18004694;//kernel.Color:4694
      if (type=="rgb") {
        //$LASTPOS=18004716;//kernel.Color:4716
        _this.makeRGBA();
        //$LASTPOS=18004731;//kernel.Color:4731
        if (_this.imageData) {
          //$LASTPOS=18004752;//kernel.Color:4752
          ...
          //$LASTPOS=18004926;//kernel.Color:4926
      ...
      //$LASTPOS=18005033;//kernel.Color:5033
      if (type=="hsl") {
        //$LASTPOS=18005055;//kernel.Color:5055
        _this.makeHSLA();
        //$LASTPOS=18005070;//kernel.Color:5070
    ...
    toString :function _trc_Color_toString() {
      ...
      //$LASTPOS=18005231;//kernel.Color:5231
      _this.makeName();
    ...
    RGBToHSL :function _trc_Color_RGBToHSL(r,g,b) {
      ...
      //$LASTPOS=18005544;//kernel.Color:5544
      ...
      //$LASTPOS=18005665;//kernel.Color:5665
      if (diff!=0) {
        //$LASTPOS=18005685;//kernel.Color:5685
        s=l<0.5?diff/(max+min):diff/(2-max-min);
        //$LASTPOS=18005749;//kernel.Color:5749
    ...
    HSLToRGB :function _trc_Color_HSLToRGB(h,s,l) {
      ...
      //$LASTPOS=18005903;//kernel.Color:5903
      if (s==0) {
        return [l,l,l];
        
      }
      //$LASTPOS=18005946;//kernel.Color:5946
      temp2 = l<0.5?l*(1+s):l+s-l*s;
      
      //$LASTPOS=18005999;//kernel.Color:5999
      temp1 = 2*l-temp2;
      
      //$LASTPOS=18006030;//kernel.Color:6030
      h/=360;
      //$LASTPOS=18006044;//kernel.Color:6044
      rtemp = (h+1/3)%1;
      gtemp = h;
      btemp = (h+2/3)%1;
      ...
      //$LASTPOS=18006159;//kernel.Color:6159
      ;
      
      while(i<3) {
        {
          //$LASTPOS=18006183;//kernel.Color:6183
  ...
  methods: {
    ...
    copy :function _trc_Panel_copy() {
      ...
      if (a.length<=4) {
        ...
        if (typeof  dx==="object") {
          //$LASTPOS=39005754;//kernel.Panel:5754
          params = dx;
          
          
        }
        //$LASTPOS=39005840;//kernel.Panel:5840
        dy=a.shift();
        //$LASTPOS=39005863;//kernel.Panel:5863
        dw=a.shift()||sw;
        //$LASTPOS=39005892;//kernel.Panel:5892
        dh=a.shift()||sh;
        //$LASTPOS=39005971;//kernel.Panel:5971
        if (sw*sh*dw*dh!=0) {
          //$LASTPOS=39006006;//kernel.Panel:6006
    ...
    convert :function _trc_Panel_convert(obj,toLayer) {
      ...
      //$LASTPOS=39006124;//kernel.Panel:6124
      if (toLayer==null) {
        //$LASTPOS=39006143;//kernel.Panel:6143
        toLayer=_this;
      }
      //$LASTPOS=39006162;//kernel.Panel:6162
      scaleY = _this.scaleY||_this.scaleX;
      
      //$LASTPOS=39006199;//kernel.Panel:6199
      if (! obj.layer) {
        throw new Error("layer not set");
        
      }
      //$LASTPOS=39006254;//kernel.Panel:6254
      if (obj.layer===_this&&toLayer===_this.layer) {
        //$LASTPOS=39006306;//kernel.Panel:6306
        dx = obj.x-_this.width/2;
        
        //$LASTPOS=39006337;//kernel.Panel:6337
        dy = obj.y-_this.height/2;
        
        //$LASTPOS=39006369;//kernel.Panel:6369
        rt = _this.rotation;
        
        //$LASTPOS=39006567;//kernel.Panel:6567
        x = _this.x+(_this.cos(rt)*dx+_this.cos(rt+90)*dy)*_this.scaleX;
        
        //$LASTPOS=39006624;//kernel.Panel:6624
        y = _this.y+(_this.sin(rt)*dx+_this.sin(rt+90)*dy)*scaleY;
        
        return {x: x,y: y,rotation: obj.rotation,scale: obj.scale,layer: toLayer};
        
      } else {
        //$LASTPOS=39006811;//kernel.Panel:6811
        if (obj.layer===_this.layer&&toLayer===_this) {
          //$LASTPOS=39006863;//kernel.Panel:6863
          rt = - _this.rotation;
          
          //$LASTPOS=39006890;//kernel.Panel:6890
          dx = obj.x-(_this.x);
          
          //$LASTPOS=39006922;//kernel.Panel:6922
          dy = obj.y-(_this.y);
          
          //$LASTPOS=39007178;//kernel.Panel:7178
          x = _this.width/2+(_this.cos(rt)*dx+_this.cos(rt+90)*dy)/_this.scaleX;
          
          //$LASTPOS=39007238;//kernel.Panel:7238
    ...
    drawLine :function _trc_Panel_drawLine(x,y,dx,dy) {
      ...
      if (dx==null&&dy==null) {
        //$LASTPOS=39007586;//kernel.Panel:7586
        if (x==null&&y==null) {
          //$LASTPOS=39007625;//kernel.Panel:7625
          _this.px=null;
          //$LASTPOS=39007633;//kernel.Panel:7633
          _this.py=null;
          return _this;
        ...
        if (_this.px==null&&_this.py==null) {
          //$LASTPOS=39007710;//kernel.Panel:7710
          _this.px=x;
          //$LASTPOS=39007715;//kernel.Panel:7715
          _this.py=y;
          return _this;
          
        }
        //$LASTPOS=39007748;//kernel.Panel:7748
        dx=x;
        //$LASTPOS=39007753;//kernel.Panel:7753
        dy=y;
        //$LASTPOS=39007758;//kernel.Panel:7758
        x=_this.px;
        //$LASTPOS=39007763;//kernel.Panel:7763
        y=_this.py;
        //$LASTPOS=39007768;//kernel.Panel:7768
        _this.px=dx;
        //$LASTPOS=39007774;//kernel.Panel:7774
        _this.py=dy;
        
      }
      //$LASTPOS=39007793;//kernel.Panel:7793
      _this.context.beginPath();
      //$LASTPOS=39007819;//kernel.Panel:7819
      _this.context.moveTo(x,y);
      //$LASTPOS=39007845;//kernel.Panel:7845
      _this.context.lineTo(dx,dy);
      //$LASTPOS=39007873;//kernel.Panel:7873
    ...
    addPattern :function _trc_Panel_addPattern(x,y,width,height) {
      ...
      //$LASTPOS=39008015;//kernel.Panel:8015
    ...
    save :function _trc_Panel_save(fn) {
      ...
      //$LASTPOS=39008331;//kernel.Panel:8331
      url = _this.buf[0].toDataURL();
      
      //$LASTPOS=39008364;//kernel.Panel:8364
      if (typeof  fn==="string") {
        //$LASTPOS=39008390;//kernel.Panel:8390
        fn=_this.file(fn);
      }
      return fn.text(url);
    },
    ...
    load :function _trc_Panel_load(fn) {
      ...
      //$LASTPOS=39008452;//kernel.Panel:8452
      if (typeof  fn==="string") {
        //$LASTPOS=39008478;//kernel.Panel:8478
        fn=_this.file(fn);
      }
      //$LASTPOS=39008496;//kernel.Panel:8496
      d = new $.Deferred();
      
      //$LASTPOS=39008525;//kernel.Panel:8525
      url = fn.text();
      
      //$LASTPOS=39008549;//kernel.Panel:8549
      element = $("<img>").css({display: "none"}).appendTo("body");
      
      //$LASTPOS=39008615;//kernel.Panel:8615
      element.on("load",(function anonymous_8634() {
        var img;
        var width;
        var height;
        
        //$LASTPOS=39008646;//kernel.Panel:8646
        img = new Image();
        
        //$LASTPOS=39008678;//kernel.Panel:8678
        img.src=element.attr('src');
        //$LASTPOS=39008718;//kernel.Panel:8718
        width = img.width;
        
        //$LASTPOS=39008750;//kernel.Panel:8750
        height = img.height;
        
        //$LASTPOS=39008816;//kernel.Panel:8816
        _this.resize(width,height);
        //$LASTPOS=39008847;//kernel.Panel:8847
        _this.context.drawImage(img,0,0);
        //$LASTPOS=39008885;//kernel.Panel:8885
        d.resolve(_this);
      }));
      //$LASTPOS=39008915;//kernel.Panel:8915
...
Tonyu.klass.define({
  ...
  methods: {
    ...
    all :function _trc_Screen_all() {
      ...
      _it_398=Tonyu.iterator(_this.layers,1);
      while(_it_398.next()) {
        l=_it_398[0];
...
Tonyu.klass.define({
  ...
  methods: {
    main :function _trc_APad_main() {
      ...
      //$LASTPOS=52000862;//kernel.APad:862
      if (! _this.auto) {
        //$LASTPOS=52000873;//kernel.APad:873
        _this.loop();
      }
    },
    ...
    draw :function _trc_APad_draw(ctx) {
      ...
      //$LASTPOS=52000968;//kernel.APad:968
      if (_this.auto) {
        //$LASTPOS=52000989;//kernel.APad:989
        _this.ix=_this.x+_this.vx*_this.outerRadius;
        //$LASTPOS=52001019;//kernel.APad:1019
        _this.iy=_this.y+_this.vy*_this.outerRadius;
        
      }
      //$LASTPOS=52001052;//kernel.APad:1052
      ctx.globalAlpha=_this.alpha/255;
      //$LASTPOS=52001084;//kernel.APad:1084
      ctx.fillStyle=_this.fillStyle+"";
      //$LASTPOS=52001117;//kernel.APad:1117
      ctx.beginPath();
      //$LASTPOS=52001139;//kernel.APad:1139
      ctx.arc(_this.ix,_this.iy,_this.innerRadius,0,Tonyu.globals.$Math.PI*2);
      //$LASTPOS=52001185;//kernel.APad:1185
      ctx.closePath();
      //$LASTPOS=52001207;//kernel.APad:1207
      ctx.fill();
      //$LASTPOS=52001224;//kernel.APad:1224
      ctx.strokeStyle=_this.strokeStyle;
      //$LASTPOS=52001258;//kernel.APad:1258
      ctx.beginPath();
      //$LASTPOS=52001280;//kernel.APad:1280
      ctx.arc(_this.x,_this.y,_this.outerRadius,0,Tonyu.globals.$Math.PI*2);
      //$LASTPOS=52001324;//kernel.APad:1324
      ctx.closePath();
      //$LASTPOS=52001346;//kernel.APad:1346
      ctx.stroke();
    },
    findTouch :function _trc_APad_findTouch() {
      ...
      //$LASTPOS=52001384;//kernel.APad:1384
      _it_504=Tonyu.iterator(Tonyu.globals.$touches,1);
      while(_it_504.next()) {
        t=_it_504[0];
        
        //$LASTPOS=52001419;//kernel.APad:1419
    ...
    loop :function _trc_APad_loop() {
      ...
      //$LASTPOS=52001544;//kernel.APad:1544
      vx = 0;
      vy = 0;
      
      //$LASTPOS=52001564;//kernel.APad:1564
      while (true) {
        //$LASTPOS=52001587;//kernel.APad:1587
        if (! _this.t||_this.t.ended) {
          //$LASTPOS=52001606;//kernel.APad:1606
          _this.t=_this.findTouch();
        }
        //$LASTPOS=52001630;//kernel.APad:1630
        if (_this.t) {
          //$LASTPOS=52001652;//kernel.APad:1652
          vx+=_this.t.vx;
          //$LASTPOS=52001675;//kernel.APad:1675
          vy+=_this.t.vy;
          
        } else {
          //$LASTPOS=52001716;//kernel.APad:1716
          vx*=_this.decay;
          //$LASTPOS=52001740;//kernel.APad:1740
          vy*=_this.decay;
          
        }
        //$LASTPOS=52001835;//kernel.APad:1835
        if (_this.getkey(_this.leftKey)) {
          //$LASTPOS=52001856;//kernel.APad:1856
          vx-=_this.keySpeed*_this.outerRadius;
        }
        //$LASTPOS=52001891;//kernel.APad:1891
        if (_this.getkey(_this.rightKey)) {
          //$LASTPOS=52001913;//kernel.APad:1913
          vx+=_this.keySpeed*_this.outerRadius;
        }
        //$LASTPOS=52001948;//kernel.APad:1948
        if (_this.getkey(_this.upKey)) {
          //$LASTPOS=52001967;//kernel.APad:1967
          vy-=_this.keySpeed*_this.outerRadius;
        }
        //$LASTPOS=52002002;//kernel.APad:2002
        if (_this.getkey(_this.downKey)) {
          //$LASTPOS=52002023;//kernel.APad:2023
          vy+=_this.keySpeed*_this.outerRadius;
        }
        //$LASTPOS=52002058;//kernel.APad:2058
        _this.d=_this.dist(vx,vy);
        //$LASTPOS=52002082;//kernel.APad:2082
        if (_this.d>_this.outerRadius) {
          //$LASTPOS=52002116;//kernel.APad:2116
          vx=vx/_this.d*_this.outerRadius;
          //$LASTPOS=52002150;//kernel.APad:2150
          vy=vy/_this.d*_this.outerRadius;
          
        }
        //$LASTPOS=52002191;//kernel.APad:2191
        _this.ix=_this.x+vx;
        //$LASTPOS=52002209;//kernel.APad:2209
        _this.iy=_this.y+vy;
        //$LASTPOS=52002227;//kernel.APad:2227
        _this.vx=_this.clamp(vx/_this.outerRadius,- 1,1);
        //$LASTPOS=52002272;//kernel.APad:2272
        _this.vy=_this.clamp(vy/_this.outerRadius,- 1,1);
        //$LASTPOS=52002317;//kernel.APad:2317
        _this.update();
...
Tonyu.klass.define({
  ...
  methods: {
    ...
    loadImages :function _trc_Boot_loadImages() {
      ...
      _it_555=Tonyu.iterator(r[0].names,2);
      while(_it_555.next()) {
        name=_it_555[0];
        val=_it_555[1];
...
Tonyu.klass.define({
  ...
  methods: {
    main :function _trc_FadeEffect_main() {
      ...
      //$LASTPOS=63000159;//kernel.FadeEffect:159
      _this.duration = _this.duration||30;
      
      switch (_this.type) {
      case "in":
        //$LASTPOS=63000228;//kernel.FadeEffect:228
        _this.alpha=255;
        //$LASTPOS=63000238;//kernel.FadeEffect:238
        _this.alphaTo=0;
        break;
        
      case "out":
        //$LASTPOS=63000275;//kernel.FadeEffect:275
        _this.alpha=0;
        //$LASTPOS=63000283;//kernel.FadeEffect:283
        _this.alphaTo=255;
        break;
        
      }
      //$LASTPOS=63000304;//kernel.FadeEffect:304
      _this.fillRect(0,0,Tonyu.globals.$screenWidth,Tonyu.globals.$screenHeight);
      //$LASTPOS=63000346;//kernel.FadeEffect:346
      _this.sa=_this.alpha;
      //$LASTPOS=63000356;//kernel.FadeEffect:356
      //$LASTPOS=63000361;//kernel.FadeEffect:361
      _this.i=0;for (; _this.i<_this.duration ; _this.i++) {
        {
          //$LASTPOS=63000387;//kernel.FadeEffect:387
          _this.progress=_this.i/(_this.duration-1);
          //$LASTPOS=63000416;//kernel.FadeEffect:416
          _this.alpha=_this.alphaTo*_this.progress+_this.sa*(1-_this.progress);
          //$LASTPOS=63000460;//kernel.FadeEffect:460
          _this.update();
        }
      }
      //$LASTPOS=63000472;//kernel.FadeEffect:472
      _this.sendEvent("complete");
      //$LASTPOS=63000495;//kernel.FadeEffect:495
      if (_this.page) {
        //$LASTPOS=63000511;//kernel.FadeEffect:511
